...
## Lab13: Загрузка моделей. Вершинный и индексный буферы.
### Task1: Работа с собственной моделью.
...
* Каждую вершину закрасить своим цветом (повторения возможны).
### Task2: Загрузить модель головы из OBJ файла.
...
### Task3: Реализовать с помощью шейдеров (через фрагментный шейдер) модели освещения.
1) без текстур по модели Фонга через вершинный шейдер - интерполяция Гуро.
2) без текстур по модели Фонга через фрагментный шейдер - интерполяция Фонга.
3) с текстурами по модели Фонга через вершинный шейдер - интерполяция Гуро.
4) с текстурами по модели Фонга через фрагментный шейдер - интерполяция Фонга.
